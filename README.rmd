...
mxdat now also comes with a suite of updated ggplot aux functions.
...
```{r}
Year_Month <- function(x) format(as.Date(x), "%Y_%B")
Cols_to_Gather <- data %>% select_if(is.numeric) %>% names() #this just gets the name of the cols and thats it as you can see below we use this object in "all_of"
...
#step by step 1 tidy 2 and then use tbl_xts and cols_to_xts the cols name, spread by what we gathered by 3 use the performance anaylytics package to calculate returnd (it requires xts format) 4  after returns have been calculated then we can go back tibble using xts_tbl but it will again be wide so we want to gather to get it tidy again
